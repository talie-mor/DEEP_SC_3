...
def char_maps(text: str):
    ...
    for i in range(len(sortext)):
        if sortext[i] not in char_to_uniq.keys():
            char_to_uniq[sortext[i]] = i
    char_to_idx = {c:i for i,c in enumerate(char_to_uniq.keys())}
...
def remove_chars(text: str, chars_to_remove):
    ...
    # ====== YOUR CODE: ======
    text_clean = list(filter(lambda c: c not in chars_to_remove, text))
    n_removed = len(text) - len(list(text_clean))
    # ========================
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    nums = torch.Tensor(range(embedded_text.size()[1]))
    result = []
    for i in range(embedded_text.size()[0]):
        result.append(idx_to_char[torch.sum(embedded_text[i, :]*nums).item()])
    # ========================
    return ''.join(result)
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    text_embed = chars_to_onehot(text, char_to_idx)
    samples = torch.stack(torch.split(text_embed, seq_len, dim=0)[:-1])
    labels = Tensor([char_to_idx[char] for char in text[1:N*seq_len+1]]).reshape(N, seq_len)
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        for i in range(n_chars-len(start_sequence)):
            feed = chars_to_onehot(c, char_to_idx)
            feed = feed.reshape(1, feed.shape[0], feed.shape[1])
            feed = feed.to(dtype=torch.float, device=device)
            y, h = model(feed, h)
            y_proba = hot_softmax(y[:,-1,:].reshape(1, y.shape[2]), 1, T)
            prediction = torch.multinomial(y_proba, 1).item()
            c = idx_to_char[prediction]
            out_text += c
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        idx = [i+b for b in range(num_batches) for i in range(0, num_batches * self.batch_size, num_batches)]
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        final_layer = nn.Linear(h_dim, out_dim, bias=True)
        for i in range(n_layers):
            x_dim = in_dim if i==0 else h_dim
            wx = [nn.Linear(x_dim, h_dim, bias=False) for _ in range(3)]
            wh = [nn.Linear(h_dim, h_dim, bias=True) for _ in range(3)]
            linears = (wx[0], wh[0], wx[1], wh[1], wx[2], wh[2], drop)
            for j, linear in enumerate(linears):
                if j==6:
                    break
                name = "wx" if j%2==0 else "wh"
                self.add_module(name+str(j)+str(i), linear)
            self.layer_params.append(linears)
        self.add_module("dropout", drop)
        self.layer_params.append((final_layer, None, None, None, None, None, None))
        self.add_module("final", final_layer)
        # ========================
