...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            r = self.train_epoch(dl_train, verbose=verbose)
            train_loss.append(r.losses)
            train_acc.append(r.accuracy)
            train_result = r
            r = self.test_epoch(dl_test, verbose=verbose)
            test_loss.append(r.losses)
            test_acc.append(r.accuracy)
            test_result = r
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss = sum([self.loss_fn(pred[:,c,:], y[:,c]) for c in range(seq_len)])
        loss.backward()
        self.optimizer.step()
        num_correct = (pred.argmax(dim=2) == y).sum()
        # ========================
